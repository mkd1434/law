...
- [x] 백엔드 단위 테스트 작성 (lawDetector.test.ts)
...
- [x] 상세 페이지 로그인 제거 - 누구나 접근 가능
- [ ] 신구법 비교 텍스트 하이라이트 및 시각화
- [ ] 모니터링 대상 관리 페이지 (추가/수정/삭제 UI)
- [x] 반응형 디자인 (모바일, 태블릿, 데스크톱)
- [x] 로딩 상태 및 에러 처리 UI
- [x] 프론트엔드 단위 테스트 작성 (DetailView.test.ts)

## Phase 4-1: 개선 작업 완료
- [x] API 엔드포인트 엄격 분리 (lawService.do vs admRulService.do)
- [x] Rate Limiting 강화 (2개씩, 1.5초 간격)
- [x] CSV 기반 Seed Data 로더 구현
- [x] 시행 예정(Upcoming) 데이터 판별 로직
- [x] 강화된 예외 처리 및 로깅
- [x] 통합 데이터 수집 테스트 완료 (18개 법령 처리)
- [x] 두 번째 체크포인트 생성 완료 (ID: b9ea4850)
...
- [x] 첫째 체크포인트 생성 완료 (ID: 9bb6b283)생성

## Phase 4-2: 추가 기능 및 최적화
- [ ] 신구법 비교 텍스트 하이라이트 및 시각화 개선
- [ ] 모니터링 대상 관리 페이지 UI (추가/수정/삭제)
- [ ] 동기화 스케줄러 스탠바이 및 총 실행 로깅
- [ ] 사용자 인증 및 권한 바인딩 완성
- [ ] 상세 화면 레이아웃 개선

## Phase 5: 환경 변수 설정 및 전체 시스템 통합 테스트
- [x] .env 파일 템플릿 생성 (.env.example)
- [x] 환경 변수 설정 (DB 주소, API Key, 포트 등)
- [x] 로컬 환경 테스트 (MySQL 연결, API 호출, UI 렌더링)
- [x] 법령 API 통합 테스트 (Rate Limiting, 예외 처리)
- [ ] 행정규칙 API 통합 테스트 (실제 API 호출)
- [x] Rate Limiting 및 Retry 로직 테스트
- [ ] 신구법 비교 데이터 정확성 검증 (AWS EC2에서 실제 데이터 수집 후)
- [ ] 전체 워크플로우 E2E 테스트

## Phase 6: GitHub 푸시 및 최종 배포 가이드 작성
- [x] GitHub 저장소 설정 (https://github.com/mkd1434/law.git)
- [x] 로컬 Git 저장소 초기화
- [x] GitHub으로 코드 푸시 (초기 및 운영 중)
- [ ] AWS EC2 배포 가이드 작성
- [ ] 최종 사용자 진회 및 내용 정리
